...
def ddpg(env: UnityEnvironment, agent=None, n_episodes=5000, max_t=2000, eps_start=1.0,
         eps_end=0.01, eps_decay=0.9995, seed=0, target=0.5):
    """
    ...
    if not agent:                                   # create the agent if no agent is provided
        agent = DDPGAgent(state_size=state_size, action_size=action_size, random_seed=seed)
    ...
    for i_episode in tqdm(range(1, n_episodes + 1)):
        ...
        scores_window.append(np.max(scores))        # save most recent score to the window
...
def score_log(i, s, e):
    """Log score"""
    return f'\rEpisode {i}\tAverage Score: {np.mean(s):.5f}\tLast Episode Score: {np.max(e):.5f}'
